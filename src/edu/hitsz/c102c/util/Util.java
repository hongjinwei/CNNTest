...
public class Util {
	 ...
	 * 
	 */
	public static double[][] matrixOp(final double[][] ma, final double[][] mb, final Operator operatorA, final Operator operatorB, OperatorOnTwo operator) {
		final int m = ma.length;
		int n = ma[0].length;
		if (m != mb.length || n != mb[0].length)
			throw new RuntimeException("两个矩阵大小不一致 ma.length:" + ma.length + "  mb.length:" + mb.length);
	...
	public static void testRot180() {
		...
		printMatrix(matrix);
		printMatrix(rot180(matrix));
		// System.out.println(rot180(matrix));
		printMatrix(matrix);
	}
